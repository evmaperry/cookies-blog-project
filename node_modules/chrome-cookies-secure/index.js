...
function convertRawToSetCookieStrings(cookies) {
	...
	cookies.forEach(function(cookie) {
		...
		let dateCreated = new Date(convertChromiumTimestampToUnix(cookie.creation_utc) * 1000);

		out += cookie.name + '=' + cookie.value + '; ';
		out += 'expires=' + tough.formatDate(dateExpires) + '; ';
		out += 'domain=' + cookie.host_key + '; ';
		out += 'path=' + cookie.path + '; ';
		out += 'created=' + tough.formatDate(dateCreated);
	...
	getDerivedKey(function (err, derivedKey) {
		...
		db.serialize(function () {
			...
			db.each(
				...
				"encrypted_value, creation_utc, is_httponly, has_expires, " +
				"is_persistent FROM cookies WHERE host_key LIKE '%" + domain +
